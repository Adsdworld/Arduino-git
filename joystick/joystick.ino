...
L'objectif est de d'allumer 4 leds selon la position du joystick, je ségmente en 4 zones mon joystick, zone 1 en haut à gauche, zone 2 en haut à droite, zone 3 en bas à droite, zone 4 en bas à gauche. 
Une fois que j'ai ces zones je sais que je dois allumer une des 2 leds de la zone. Pour déterminer laquelle, je fais un produit scalaire pour obtenir l'angle, j'obtiens une valeur qui pass par 1 | 0 | 1, qui couvre une distance de 180°.

Tout d'abord je regarde si mon joystick est vers le bas ou vers le haut
je calcule l'angle
si l'angle est <0.5 on est dans la zone basse  1 | 0.5 0 0.5 | 1
...
Pour savoir si j'allume la led droite ou gauche, je regarde les coordonnées en les comparant au centre, pour connaitre si on est à droite ou à gauche du centre
...
if (xValue>=(X_centre-1) && xValue<=(X_centre+1) && yValue>=(Y_centre-1) && yValue<=(Y_centre+1)){     //jostick au "0"
  ...
  if (xValue>(X_centre+1)){                                                                            //joystick vers le bas
    ...
    if (angle<0.5){                                                                                    //joystick vers le bas dans la zone basse
      digitalWrite(bas, HIGH);
      digitalWrite(haut, LOW);                            
      digitalWrite(droite, LOW);
      digitalWrite(gauche, LOW);}else{

      if (Vy<=0){                                                                                      //joystick vers le bas vers la droite
        if (angle>=0.5){
      ...
      }else{                                                                                           //joystick vers le bas vers la gauche
        if (angle>=0.5){
    ...
    if (xValue<(X_centre-1)){                                                                          //joystick vers le haut
      ...
      if (angle<0.5){                                                                                  //joystick vers le haut dans la zone haute
